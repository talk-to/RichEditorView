...
extension RichEditorView {
    ...
    fileprivate func adjustContentOffsetFromCaretPosition() {
        ...
        if let caretPositionNumeric = data!["height"] as? NSNumber {
            ...
            if newLine {
                if (caretFloat + 24.0) > (scrollView.bounds.size.height) {
                    let bottomOffset = CGPoint(x: 0, y: (CGFloat(caretPositionNumeric.floatValue) - (scrollView.bounds.size.height + scrollView.contentOffset.y)) + scrollView.contentOffset.y + 28.0)
                    scrollView.setContentOffset(bottomOffset, animated: true)
                } else if (CGFloatCaretPositionNumeric < scrollView.contentOffset.y) && scrollView.contentOffset.y > 0 {
                    let bottomOffset = CGPoint(x: scrollView.contentOffset.x, y: CGFloatCaretPositionNumeric)
                    scrollView.setContentOffset(bottomOffset, animated: true)
                }
            } else {
                if CGFloatCaretPositionNumeric + 24.0 > scrollView.bounds.size.height {
                    let bottomOffset = CGPoint(x: 0, y: ((CGFloatCaretPositionNumeric - scrollView.bounds.size.height) + scrollView.contentOffset.y + 28.0))
                    scrollView.setContentOffset(bottomOffset, animated: true)
                } else if (CGFloatCaretPositionNumeric < 0) {
                    var amount = scrollView.contentOffset.y + CGFloatCaretPositionNumeric
                    amount = amount < 0 ? 0 : amount
                    let bottomOffset = CGPoint(x: scrollView.contentOffset.x, y: amount)
                    scrollView.setContentOffset(bottomOffset, animated: true)
                }
            }
        }
